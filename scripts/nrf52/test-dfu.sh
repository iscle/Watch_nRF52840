...
# Watch sets and advertises "Watch" as its bluetooth device name. 
...
sudo ./newtmgr conn add Watch type=ble connstring="peer_name=Watch"
# Connection profile Watch successfully added

# Connect to Watch and list firmware images
sudo ./newtmgr image list -c Watch --loglevel debug

# Step 2: Uploading an Image to the Device
# We upload my_sensor_app_1.1.img (downloaded from https://github.com/lupyuen/Watch-rust-mynewt/releases/tag/v4.1.2)

# sudo ./newtmgr image upload -c Watch \
#    ~/my_sensor_app_1.1.img 

# Connect to Watch and list firmware images
# sudo ./newtmgr image list -c Watch
...
# sudo ./newtmgr image confirm -c Watch
...
Connection profile Watch successfully added
+ sudo ./newtmgr image list -c Watch --loglevel debug
DEBU[2020-05-19 04:46:13.693] Using connection profile: name=Watch type=ble connstring=peer_name=Watch 
...
+ sudo ./newtmgr image upload -c Watch /home/ubuntu/my_sensor_app_1.1.img
...
+ sudo ./newtmgr image test -c Watch 66a23f4f8f5766b5150711eb8c7c4be326cebabef37429fd21879f6e0eacffe5
Images:
 image=0 slot=0
    version: 1.0.0
    bootable: true
