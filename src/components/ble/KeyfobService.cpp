...
#include "KeyfobService.h"

int MSCallback(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg) {
  auto musicService = static_cast<Watch::Controllers::KeyfobService *>(arg);
  return musicService->OnCommand(conn_handle, attr_handle, ctxt);
}

Watch::Controllers::KeyfobService::KeyfobService(Watch::System::SystemTask &system) : m_system(system) {
...
void Watch::Controllers::KeyfobService::Init() {
  int res = 0;
...
int Watch::Controllers::KeyfobService::OnCommand(uint16_t conn_handle, uint16_t attr_handle,
...
std::string Watch::Controllers::KeyfobService::getAlbum() {
  return albumName;
}

std::string Watch::Controllers::KeyfobService::getArtist() {
  return artistName;
}

std::string Watch::Controllers::KeyfobService::getTrack() {
  return trackName;
}

bool Watch::Controllers::KeyfobService::isPlaying() {
  return playing;
}

float Watch::Controllers::KeyfobService::getPlaybackSpeed() {
  return playbackSpeed;
}

void Watch::Controllers::KeyfobService::event(char event) {
...
int Watch::Controllers::KeyfobService::getProgress() {
  return trackProgress;
}

int Watch::Controllers::KeyfobService::getTrackLength() {
  return trackLength;
}
