...
int NAVCallback(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg) {
  auto navService = static_cast<Watch::Controllers::NavigationService *>(arg);
  return navService->OnCommand(conn_handle, attr_handle, ctxt);
}

Watch::Controllers::NavigationService::NavigationService(Watch::System::SystemTask &system) : m_system(system) {
...
void Watch::Controllers::NavigationService::Init() {
  int res = 0;
...
int Watch::Controllers::NavigationService::OnCommand(uint16_t conn_handle, uint16_t attr_handle,
...
std::string Watch::Controllers::NavigationService::getFlag()
{
    return m_flag;
}

std::string Watch::Controllers::NavigationService::getNarrative()
{
    return m_narrative;
}

std::string Watch::Controllers::NavigationService::getManDist()
{
    return m_manDist;
}

int Watch::Controllers::NavigationService::getProgress()
{
    return m_progress;
}
