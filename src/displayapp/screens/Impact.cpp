...
static void event_handler(lv_obj_t * obj, lv_event_t event) {
  Impact* screen = static_cast<Impact *>(obj->user_data);
  screen->OnObjectEvent(obj, event);
}


Impact::Impact(DisplayApp* app,
  ...
  lv_obj_align(label_time, lv_scr_act(), LV_ALIGN_IN_TOP_LEFT, 0, 0);
    ...
    labelImpact = lv_label_create(lv_scr_act(), nullptr);
    lv_label_set_text(labelImpact, "IMPACT");                   /*Set the labels text*/
    lv_obj_align(labelImpact, lv_scr_act(), LV_ALIGN_IN_BOTTOM_MID, 0, -110);
    ...
    lv_label_set_style(labelImpact, LV_LABEL_STYLE_MAIN,labelRelStyle);
...
bool Impact::Refresh() {
 ...
 char imStr[3];
  sprintf(imStr,"%02d",batteryController.getimpactzz());
   lv_label_set_text(labelButtonHelp,imStr);
...
void Impact::OnObjectEvent(lv_obj_t *obj, lv_event_t event) {
