...
float Kx126::Process() {
  uint8_t test =0;
  spi.Read(&KX126_WHO_AM_I, 1,&test, 1);
  if(test != 0x38) return 0;
...
Kx126::Values Kx126::ProcessTest() {
  ...
  if(test != 0x38) return {0,0,0,0};
	...
	uint8_t valuex =x1/409.6f;
	uint8_t valuey =y1/409.6f;
	uint8_t valuez =z1/409.6f;
